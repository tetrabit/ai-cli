...
From a local checkout, `./install.sh` installs the local `ai.sh` next to it. Use `./install.sh --remote` to force downloading the latest `main` version instead.
...
| Pi Coding Agent | npm version check | npm version check |
| Pi vs Claude Code | git clone/pull + `bun install` | git clone/pull + `bun install` |
| Hermes Agent | official installer when missing, otherwise `hermes update` | official installer when missing, otherwise `hermes update` |
| Oh My Codex | npm version check + `omx doctor` | npm version check + `omx doctor` |
...
Shows remaining usage as percentage bars or instruction guidelines for the selected supported providers:

- **Claude Code:** 5-hour and 7-day limits (Linux/macOS)
- **Codex CLI:** 5-hour and 7-day limits (Linux/macOS)
- **Antigravity CLI:** Captures the real interactive `agy /usage` panel through tmux and converts its model percentages into ai-cli rows.
...
==> Antigravity CLI...
  Gemini 3.5 Flash (Medium) [--------------------]    0.0% left  resets in 24m
  Claude Sonnet 4.6 (Thinking) [####################]  100.0% left
